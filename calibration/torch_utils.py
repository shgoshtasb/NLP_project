...
def get_equal_bins(probs, num_bins=10):
    ...
    if bins[-1] != 1.:
        bins = torch.cat([bins.to(torch.float32), torch.ones(1).to(torch.float32)], dim=0)
    return bins

def get_equal_prob_bins(probs, num_bins=10):
    return torch.linspace(0., 1., steps=num_bins+1)[1:]
...
def fast_bin(prob_label, bins):
    ...
    splits = searchsorted(sorted_bins, torch.arange(1, bins.shape[0]))
...
def plugin_ce(binned_data, power=2):
    ...
    bin_errors = torch.tensor(list(map(bin_error, binned_data)))
...
def unbiased_square_ce(binned_data):
    ...
    def bin_error(data):
        ...
        label_values = torch.tensor(list(map(lambda x: x[1], data)))
...
def normal_debiased_ce(binned_data, power=1, resamples=1000):
    bin_sizes = torch.tensor(list(map(len, binned_data)))
    if bin_sizes.min() <= 1:
    ...
    label_means = torch.tensor(list(map(lambda b: b[:, 1].mean() for b in binned_data)))
    label_stddev = torch.sqrt(label_means * (1 - label_means) / bin_sizes)
    model_vals = torch.tensor(list(map(lambda b: b[:, 0].mean() for b in binned_data)))
...
def eval_top_mse(calibrated_probs, probs, labels):
    ...
    return torch.mean(torch.square(calibrated_probs - correct))
...
def eval_marginal_mse(calibrated_probs, probs, labels):
    ...
    return torch.mean(torch.square(calibrated_probs - labels_one_hot)) * calibrated_probs.shape[1] / 2.0


def resample(data):
    indices = torch.tensor(np.random.choice(np.arange(data.shape[0]), size=data.shape[0], replace=True))
    return data[indices]
...
def get_platt_scaler(model_probs, labels):
    ...
    eps = 1e-12
    model_probs = model_probs.numpy().astype(dtype=np.float64).reshape(-1,)
    ...
    def calibrator(probs):
        ...
        output = 1 / (1 + np.exp(-x))
        return torch.tensor(output).reshape(-1, 1)
    return calibrator
...
def get_histogram_calibrator(model_probs, values, bins):
    ...
    binned_values = torch.zeros(model_probs.shape[0], bins.shape[0])
    ...
    safe_mean = (bins[1:] + bins[:-1]) / 2.0
    safe_mean = torch.cat([torch.tensor([bins[0] / 2.0]), safe_mean], dim=0)
...
def get_labels_one_hot(labels, k):
    labels_one_hot = torch.zeros((labels.shape[0], k))
    labels_one_hot.scatter_(1, labels.to(torch.long), 1)
